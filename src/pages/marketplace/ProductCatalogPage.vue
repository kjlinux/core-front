...
function confirmAddToCart() {
  ...
  toast.showSuccess(`${selectedProduct.value.name} ${t('marketplace.addedToCart')}`)
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ t('marketplace.catalogTitle') }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ t('marketplace.catalogSubtitle') }}</p>
      </div>
      <AppButton variant="secondary" @click="router.push('/marketplace/cart')">
        {{ t('marketplace.cart') }} ({{ cartStore.itemCount }})
      </AppButton>
    </div>

    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
      <AppSearchInput v-model="search" :placeholder="t('marketplace.searchProduct')" class="flex-1" />
    </div>
        ...
        <div class="p-4 space-y-3">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <AppBadge :variant="product.stockQuantity > 0 ? 'success' : 'danger'">
              {{ product.stockQuantity > 0 ? t('marketplace.inStock') : t('marketplace.outOfStock') }}
            </AppBadge>
    ...
    <!-- Add to cart modal -->
    <AppModal v-if="selectedProduct" v-model="showAddModal" :title="`${t('marketplace.addToCart')} - ${selectedProduct.name}`" size="sm">
      <div class="space-y-4">
        ...
        <div>
          <label class="text-sm font-medium text-gray-700">{{ t('marketplace.quantity') }} {{ selectedProduct.minQuantity }})</label>
        ...
        <p class="text-sm text-gray-600">
          {{ t('marketplace.total') }} <span class="font-bold text-primary">{{ formatPrice(selectedProduct.price * quantity, selectedProduct.currency) }}</span>
        </p>
      </div>
      <template #footer>
        <div class="flex justify-end gap-3">
          <AppButton variant="secondary" @click="showAddModal = false">{{ t('common.cancel') }}</AppButton>
          <AppButton variant="primary" @click="confirmAddToCart">{{ t('marketplace.addToCart') }}</AppButton>
        </div>
      </template>
    </AppModal>
  </div>
