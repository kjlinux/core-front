...
import { ArrowLeftIcon, KeyIcon, PhoneIcon } from '@heroicons/vue/24/outline'
    ...
    <div v-if="detail" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <AppCard title="Utilisateurs" class="lg:col-span-2">
        <div class="divide-y divide-gray-100">
          <div v-for="u in detail.users" :key="u.id" class="py-3 flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-gray-900">{{ u.name }}</p>
              <p class="text-xs text-gray-500">{{ u.email }} · {{ u.role }}</p>
            </div>
